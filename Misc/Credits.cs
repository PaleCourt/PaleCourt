    ...
    public class CreditsController : MonoBehaviour
    {
        ...
        private void FixTranslations()
        {
            foreach(Text text in _creditsParent.parent.GetComponentsInChildren<Text>(true))
            {
                ...
                string textKey = "";
                if (textGo.name.Contains("Body"))
                {
                    // it's parent->part->body
                    textKey = textGo.transform.parent.parent.gameObject.name + "-" + textGo.transform.parent.gameObject.name + "-" + textGo.name;
                }
                else
                {
                    // it's parent->part
                    textKey = textGo.transform.parent.gameObject.name + "-" + textGo.name;
                }
                text.text = Language.Language.Get(textKey, "PaleCourtCredits");
                if (text.text == "")
                {
                    text.text = $"\"{textKey}\" - \"PaleCourtCredits\"";
                }
            }
        }
