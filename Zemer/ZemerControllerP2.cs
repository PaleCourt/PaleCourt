    ...
    public class ZemerControllerP2 : MonoBehaviour
    {
        ...
        private IEnumerator NailLaunch()
        {
            ...
            IEnumerator LaunchUp(float rot, GameObject nail, bool isRepeat=false)
            {
                ...
                Vector2 zem = gameObject.transform.position;
                float oldDir = dir;
                yield return FlashRepeat(hero, AfterAirNailGrabDelay / 3f);
                _anim.enabled = true;
                yield return new WaitWhile(() => _anim.GetCurrentFrame() < 5);
                nail.SetActive(true);
                dir = FaceHero();
        ...
        private IEnumerator SweepDash()
        {
            ...
            IEnumerator AltZSpin(ParticleSystem psGrass)
            {
                _anim.enabled = true;
        ...
        private IEnumerator DoSpiralPassage()
        {
            ...
            IEnumerator FlashAppear(Vector2 targ)
            {
                ...
                controller.transform.localScale *= 0.58f;
        ...
        private void HealthManager_TakeDamage(On.HealthManager.orig_TakeDamage orig, HealthManager self, HitInstance hitInstance)
        {
            if (self.name.Contains("Zemer"))
            {
                    ...
                    (DoneFrenzyAtt == 1 && _hm.hp < 0.35f * Phase2HP))
                {
                    ...
                    var grass = GameObject.Find("GrassParticle");
        ...
        IEnumerator FlashRepeat(Vector3 targ, float timer = 0.2f)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                _ap.Spawn = glow;
                PlayAudioClip(i % 2 == 0 ? "AudBasicSlash1" : "AudBasicSlash2", _ap, 0.5f, 0.8f);
                _ap.Spawn = gameObject;
